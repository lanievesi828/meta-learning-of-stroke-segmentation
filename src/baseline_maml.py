...
def main():
    ...
    # 数据源是你新预处理流程的输出
    # 请确保这个路径是正确的！
    data_root = "/root/autodl-tmp/maml_data_functional_tasks" 
    
    # 为这个基线实验创建一个新的、独立的输出目录
    output_dir = "/root/autodl-tmp/maml_data_functional_tasks_baseline_train"
    ...
    train_json_path = os.path.join(data_root, "train_functional_meta_tasks.json")
    val_json_path = os.path.join(data_root, "val_functional_meta_tasks.json")   
    ...
    batch_size = 16 * max(1, num_gpus) # 如果没有GPU，则为16*1=16
    print(f"检测到 {num_gpus} 个GPU，设置总 batch_size 为 {batch_size}")

    train_dataloader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, num_workers=4 * max(1, num_gpus), pin_memory=True)
    val_dataloader = DataLoader(val_dataset, batch_size=batch_size, shuffle=False, num_workers=4 * max(1, num_gpus), pin_memory=True)
    
    print(f"训练样本数: {len(train_dataset)}, 验证样本数: {len(val_dataset)}")

    # --- 3. 初始化模型、损失、优化器 ---
    # 【修改】设备定义
    # DataParallel 会自动处理设备分配，我们只需要确认CUDA可用
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    
    # a. 首先在CPU上实例化原始模型
    # 这是一个好习惯，避免一开始就在特定GPU上创建张量
    model = MAML_BiFPN_Transformer_UNet(in_channels=1, classes=1)
    
    # b. 【核心修改】使用 DataParallel 包装模型
    # 这个判断是正确的，我们来完善它
    if torch.cuda.device_count() > 1:
        print(f"使用 {torch.cuda.device_count()} 个GPU进行并行训练!")
        # DataParallel 会将模型和数据自动分配到所有可用的GPU上
        model = nn.DataParallel(model)
    
    # c. 将模型（或包装后的模型）移动到主设备
    model.to(device)
    
    criterion = MAML_BCEWithLogitsAndEdgeEnhancedDiceLoss(bce_weight=0.1) # 沿用旧脚本的bce_weight
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        if val_dice > best_val_dice:
            ...
            torch.save(state_to_save, os.path.join(checkpoints_dir, 'best_model.pth'))
        
        # 保存检查点时也一样
        if (epoch + 1) % 10 == 0:
            # 同样，保存 model.module 的状态
            state_dict_to_save = model.module.state_dict() if isinstance(model, nn.DataParallel) else model.state_dict()
            # 创建检查点字典时使用这个解包后的state_dict
            checkpoint = {
                'epoch': epoch + 1,
            ...
            torch.save(checkpoint, os.path.join(checkpoints_dir, f'checkpoint_epoch_{epoch+1}.pth'))
            print(f"Checkpoint saved at {os.path.join(checkpoints_dir, f'checkpoint_epoch_{epoch+1}.pth')}")
